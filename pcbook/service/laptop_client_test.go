...
func requireSameLaptop(t *testing.T, laptop, foundLaptop *pb.Laptop) {
	// require.Equal(t, laptop, foundLaptop)
	jsonLaptop, err := serializer.ProtobufToJson(laptop)
	require.NoError(t, err)

	jsonFoundLaptop, err := serializer.ProtobufToJson(foundLaptop)
	require.NoError(t, err)

	require.Equal(t, jsonLaptop, jsonFoundLaptop)
}
...
func TestClientCreateLaptop(t *testing.T) {
	...
	// check that the laptop is stored on the server side.
	foundLaptop, err := laptopStore.Find(res.Id)
	require.NoError(t, err)
...
func TestClientSearchLaptop(t *testing.T) {
	...
	require.Equal(t, len(expectedIDs), match)
}
